  ...
  return (
    <main id="main-content" className="min-h-screen px-5 py-8 sm:px-6 sm:py-12">
      <section className="mx-auto max-w-6xl elvyn-enter">
        <header className="flex flex-col gap-6 rounded-[24px] border border-black/[.07] bg-white/55 p-5 shadow-[0_16px_50px_rgba(24,24,27,.045)] backdrop-blur sm:flex-row sm:items-start sm:justify-between sm:p-6 dark:border-white/[.08] dark:bg-white/[.025]">
          <div>
        ...
        <div className="grid gap-5 md:grid-cols-2">
          <SectionCard><ProfileSummary profile={profile} email={data.user.email ?? 'No email available'} /><div className="mt-4"><Link href="/dashboard/profile" className="inline-flex min-h-11 items-center text-sm font-medium text-[#6357dc] transition hover:opacity-75 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#6d5dfb]/50 focus-visible:ring-offset-2 dark:text-[#a29cff]">Edit profile →</Link></div></SectionCard>
        ...
        <div className="mt-5 grid gap-5 md:grid-cols-2">
          <SectionCard><div className="flex items-center justify-between"><h2 className="font-semibold tracking-tight">Goals</h2><Link href="/dashboard/goals" className="inline-flex min-h-11 items-center text-xs font-medium text-[#6357dc] hover:opacity-75 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#6d5dfb]/50 focus-visible:ring-offset-2 dark:text-[#a29cff]">View all →</Link></div>{goals.length === 0 ? <p className="mt-5 text-sm text-black/55 dark:text-white/55">No goals yet.</p> : <div className="mt-4 space-y-3">{goals.slice(0, 3).map((goal) => <div key={goal.id} className="elvyn-card flex items-center justify-between gap-3 rounded-xl p-3"><span className="truncate text-sm">{goal.title}</span><span className="shrink-0 rounded-full bg-[#f0edff] px-2.5 py-1 text-[11px] font-medium capitalize text-[#6357dc] dark:bg-[#1b1930] dark:text-[#a29cff]">{goal.status}</span></div>)}</div>}</SectionCard>
          <SectionCard><div className="flex items-center justify-between"><h2 className="font-semibold tracking-tight">Tasks</h2><Link href="/dashboard/tasks" className="inline-flex min-h-11 items-center text-xs font-medium text-[#6357dc] hover:opacity-75 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#6d5dfb]/50 focus-visible:ring-offset-2 dark:text-[#a29cff]">View all →</Link></div>{tasks.length === 0 ? <p className="mt-5 text-sm text-black/55 dark:text-white/55">No tasks yet.</p> : <div className="mt-4 space-y-3">{tasks.slice(0, 3).map((task) => <div key={task.id} className="elvyn-card flex items-center justify-between gap-3 rounded-xl p-3"><span className="truncate text-sm">{task.title}</span><span className="shrink-0 rounded-full bg-black/[.045] px-2.5 py-1 text-[11px] font-medium capitalize text-black/55 dark:bg-white/[.06] dark:text-white/55">{task.status.replace('_', ' ')}</span></div>)}</div>}</SectionCard>
        </div>
      </section>
    </main>
  )
...
function Stat({ label, value }: { label: string; value: number }) {
  return <div className="rounded-xl border border-black/[.05] bg-black/[.025] p-3 transition hover:-translate-y-0.5 hover:bg-[#f0edff]/70 sm:p-4 dark:border-white/[.07] dark:bg-white/[.035] dark:hover:bg-[#1b1930]/80"><p className="text-xl font-semibold tracking-tight sm:text-2xl">{value}</p><p className="mt-1 text-[11px] leading-tight text-black/50 sm:text-xs dark:text-white/50">{label}</p></div>
}
